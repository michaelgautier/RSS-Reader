...
update_visuals(const ns_visual_model::activity& activity_past, ns_visual_model::activity& activity_now)
{
	...
	if(screen_changed)
	{
		...
		if(screen)
		{
			...
			ns_visual_model::arrange_elements(ns_visual_model::unit_type_arrange_strategem::canvased_vertical_stacks, screen, Fl::w(), Fl::h());
		...
		if(ns_diag::verbosity_level >= 4 && (activity_past.screen_name != activity_now.screen_name))
		{
			std::cout << "tracked screen changed from " << activity_past.screen_name << " to " << activity_now.screen_name << "\n";
		}
	}
...
timed_execute(void* data)
{
	...
	if(data)
	{
		std::cout << "timeout data supplied but not processed.\n";
	}

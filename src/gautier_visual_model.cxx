...
static void arrange_elements_canvased_vertical_stacks(ns_visual_model::unit_type_screen* screen, const ns_visual_model::type_size_ar screen_width, const ns_visual_model::type_size_ar screen_height);
...
void 
ns_visual_model::
arrange_elements(const ns_visual_model::unit_type_arrange_strategem arrange_type, ns_visual_model::unit_type_screen* screen, const type_size_ar screen_width, const type_size_ar screen_height)
{
	if(arrange_type == ns_visual_model::unit_type_arrange_strategem::canvased_vertical_stacks)
	{
		arrange_elements_canvased_vertical_stacks(screen, screen_width, screen_height);
	}
...
static 
void 
arrange_elements_canvased_vertical_stacks(ns_visual_model::unit_type_screen* screen, const ns_visual_model::type_size_ar screen_width, const ns_visual_model::type_size_ar screen_height)
{
	const ns_visual_model::type_size_ar min_v = 0;

	list_type_follows_count 
		follows_count
	;

	const auto se_type_vertical_stack = ns_visual_model::unit_type_screen_element_class::vertical_stack;

	bool parent_se_type_is_vertical_stack = false;
			...
			else
			{
				if(parent_se.name != follows)
				{
					...
					if(found_se)
					{
						parent_se = *found_se;
						parent_se_type = parent_se.element_type;
						parent_se_type_is_vertical_stack = (parent_se_type == se_type_vertical_stack);
				...
				if(area.x <= min_v)
				{
					if(parent_se_type_is_vertical_stack)
					{
